...
def hospital_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        hospital.save()
        messages.success(request, f'"{hospital.name}" o\'chirildi.')
        return redirect('hospital_list')
    return render(request, 'frontend/hospital_confirm_delete.html', {'hospital': hospital})
...
def disease_search(request):
    """Kasallik nomi bo'yicha kasalxonalarni qidirish (AJAX)"""
    query = request.GET.get('q', '').strip()
    results = []

    # Kasallik → mutaxassislik/lavozim mapping
    DISEASE_MAP = {
        "ko'z": ['shifokor'],
        'koz': ['shifokor'],
        'ko`z': ['shifokor'],
        'ko z': ['shifokor'],
        'glaukoma': ['shifokor'],
        'katarakta': ['shifokor', 'jarroh'],
        'yurak': ['kardiolog', 'jarroh'],
        'infarkt': ['kardiolog'],
        'yurak kasalligi': ['kardiolog'],
        'bosim': ['kardiolog', 'shifokor'],
        'miya': ['nevropatolog', 'jarroh'],
        'insult': ['nevropatolog'],
        "bosh og'rig'i": ['nevropatolog', 'shifokor'],
        'tish': ['stomatolог'],
        "tish og'rig'i": ['stomatolог'],
        'stomatologiya': ['stomatolог'],
        'bolalar': ['pediatr'],
        'bola': ['pediatr'],
        'pediatr': ['pediatr'],
        'psixologik': ['psixiatr'],
        ...
        'operatsiya': ['jarroh'],
        'suyak': ['jarroh'],
        'qon': ['laborant', 'shifokor'],
        'diabet': ['shifokor', 'kardiolog'],
        'qand kasalligi': ['shifokor'],
        'buyrak': ['shifokor', 'jarroh'],
        'siydik': ['shifokor'],
        'teri': ['shifokor'],
        'jigar': ['shifokor', 'jarroh'],
        'oshqozon': ['shifokor', 'jarroh'],
        'anesteziya': ['anesteziolog'],
        'rentgen': ['radiolog'],
        'tahlil': ['laborant'],
        'nevrologiya': ['nevropatolog'],
    ...
    if query:
        ...
        # 1) Mos lavozimlarga ega xodimlar orqali kasalxonalarni topish
        matched_positions = []
        for keyword, positions in DISEASE_MAP.items():
            if keyword in query_lower:
                matched_positions.extend(positions)

        # 2) Kasalxonaning nomi/manzilida ham qidirish + specialization da
        hospital_ids = set()

        if matched_positions:
            staff_hospitals = Staff.objects.filter(
                is_active=True,
                position__in=matched_positions
            ).values_list('hospital_id', flat=True)
            hospital_ids.update(staff_hospitals)

        # Kasalxona nomi yoki manzilida qidiruv
        name_match = Hospital.objects.filter(
            is_active=True
        ).filter(
            Q(name__icontains=query) | Q(address__icontains=query)
        ).values_list('id', flat=True)
        hospital_ids.update(name_match)

        # Specialization bo'yicha qidiruv
        spec_match = Staff.objects.filter(
            is_active=True,
            specialization__icontains=query
        ).values_list('hospital_id', flat=True)
        hospital_ids.update(spec_match)

        hospitals = Hospital.objects.filter(
            id__in=hospital_ids, is_active=True
        ).annotate(sc=Count('staff')).order_by('-sc')[:10]

        for h in hospitals:
            # Mos keladigan mutaxassislar sanasi
            spec_staff = Staff.objects.filter(
                hospital=h, is_active=True,
                position__in=matched_positions if matched_positions else []
            ).count()
            results.append({
                'id': h.id,
                'name': h.name,
                'region': h.get_region_display(),
                'type': h.get_hospital_type_display(),
                'address': h.address,
                'phone': h.phone,
                'staff_count': h.sc,
                'spec_count': spec_staff,
            })
